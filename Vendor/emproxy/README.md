...
After a responder restart, stale client DATA can return `NoCurrentSession`. Only this error triggers one standard server-initiated handshake to the localhost sender, without authenticating or pinning that sender. The WireGuard in-progress guard suppresses duplicate initiations. Local packet-loss recovery is not guaranteed by this one-shot path. Rejection diagnostics expose a fixed category only, never raw peer messages or keys. Snapshot diagnostics also expose worker-tick deltas, UDP receive/send failure counts with only the latest raw OS errno, and accepted inner TCP SYN/SYN-ACK/RST totals split by source port `49152` or another source port. They contain no payload, peer address, key or coordinate data. A loopback regression exercises the production handler across a responder restart and confirms authenticated reflection resumes.
