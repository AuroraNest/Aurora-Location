...
后续用户反馈: 保留会话版在切 App/锁屏后仍恢复; 纯移动网络失败时收到 UDP 5、拒绝 5, 旧诊断不能区分拒绝类别. 新版增加固定拒绝分类, 并修复已独立复现的 responder 重建后旧 WireGuard session DATA 被拒绝的问题. 此问题可能解释部分重连失败, 但尚未证明是现场蜂窝握手失败的唯一根因. 新增后台位置监测已获用户同意, 两项改动都须重新真机验收.
